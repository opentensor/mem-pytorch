...
from mem_pytorch.flash_cosine_sim_attention import plain_cosine_sim_attention, flash_cosine_sim_attention
        ...
        **kwargs
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                Attention(dim, dim_head = dim_head, heads = heads, causal = causal, use_cuda_kernel= use_cuda_kernel, scale = attn_scale, **kwargs),
                nn.LayerNorm(dim),
                FeedForward(dim),
                nn.LayerNorm(dim),
            ]))
